...
TAGS_OPENED_TEXT = "Открыты метки."
...
NOTE_CONTEXT_LOST_TEXT = "Не удалось продолжить заметку. Откройте заявку ещё раз."
...
NOTE_MODE_COMMAND_BLOCK_TEXT = (
    "Сейчас открыто создание заметки. Отправьте текст заметки или нажмите «Отмена»."
)
REASSIGN_TARGET_PROMPT_TEXT = "Укажите Telegram ID нового оператора."
NOTE_PROMPT_TEXT = "Отправьте внутреннюю заметку по заявке."
...
def build_notes_opened_text(public_number: str) -> str:
...
def build_note_saved_text(public_number: str) -> str:
    return f"Заметка по заявке {public_number} сохранена."
