...
def test_ai_settings_routes_are_super_admin_only(tmp_path: Path) -> None:
    ...
    response = _request_ai_settings(gateway=gateway, static_dir=tmp_path, role=UserRole.OPERATOR)

    assert response.status_code == HTTPStatus.FORBIDDEN
    assert gateway.get_called is False

    response = _request_ai_settings(
        gateway=gateway,
        static_dir=tmp_path,
...
def _request_ai_settings(
    *, gateway: StubAISettingsGateway, static_dir: Path, role: UserRole
) -> Any:
    app = create_mini_app(
    ...
    return asyncio.run(_get_asgi(app, "/api/admin/ai-settings"))
